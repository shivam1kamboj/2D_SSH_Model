...
for j, vector in enumerate(Vec.T[:10]):
    plt.plot(((abs(vector).reshape(m, 2 * m)) ** 2)[:, 0], label='Correspond to eigen_Value=' + str(EI[igens][j]))
...
for j, vector in enumerate(Vec.T[10:]):
    plt.plot(((abs(vector).reshape(m, 2 * m)) ** 2)[:, -1], label='Correspond to eigen_Value=' + str(EI[igens][10:][j]))
    plt.legend(loc='best', fontsize=7)
